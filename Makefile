...
WERLD_CLIENT_BROWSER_GIT_REPOSITORY_URL := https://github.com/mpereira/werld-client-browser
...
deploy: pull_git_modules build_werld_client_browser build
	-@git add $(WERLD_CLIENT_BROWSER_ASSETS_FILES_AND_DIRECTORIES) $(WERLD_CLIENT_BROWSER_BUILD_FILES_AND_DIRECTORIES) $(WERLD_CLIENT_BROWSER_PATH) && git commit -m "$(WERLD_CLIENT_BROWSER_GIT_REPOSITORY_URL)/commit/$(shell cd $(WERLD_CLIENT_BROWSER_PATH) && git rev-parse HEAD)"
